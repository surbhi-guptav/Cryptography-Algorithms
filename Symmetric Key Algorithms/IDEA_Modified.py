class IDEA:
    ...
    def print_round_results(self, p1, p2, p3, p4, round_number):
        keys = self._keys[round_number]
        k1, k2, k3, k4, k5, k6 = keys
        ...
        p2 = self.add_mod(p2, k2)
        p3 = self.add_mod(p3, k3)
        print(f"Round {round_number} - Step 1:")
        print(f"p1: {hex(p1)}")
        print(f"p2: {hex(p2)}")
        print(f"p3: {hex(p3)}")
        print(f"p4: {hex(p4)}")
        ...
        t1 = self.mul_mod(k6, x)
        t2 = self.add_mod(t0, t1)
        print(f"Round {round_number} - Step 2:")
        print(f"x: {hex(x)}")
        print(f"t0: {hex(t0)}")
        print(f"t1: {hex(t1)}")
        print(f"t2: {hex(t2)}")
        ...
        p2 = p3 ^ t1
        p3 = a
        print(f"Round {round_number} - Step 3:")
        print(f"p1: {hex(p1)}")
        print(f"p2: {hex(p2)}")
        print(f"p3: {hex(p3)}")
        print(f"p4: {hex(p4)}")
    ...
    def encrypt(self, plain):
        ...
        for i in range(8):
            keys = self._keys[i]
            p1, p2, p3, p4 = self.round(p1, p2, p3, p4, keys)
            self.print_round_results(p1, p2, p3, p4, i)
        ...
        y3 = self.add_mod(p2, k3)
        y4 = self.mul_mod(p4, k4)
        print("Final Output Transformation:")
        print(f"y1: {hex(y1)}")
        print(f"y2: {hex(y2)}")
        print(f"y3: {hex(y3)}")
        print(f"y4: {hex(y4)}")
...
# Create an IDEA instance with a key
key = 0x68656d72616a20736972  #Hexadecimal value for "Hemarjsir":68656d72616a20736972 
idea = IDEA(key)

# Convert the string "thisisourgroup" to binary (64 bits) and pad if necessary
plaintext = "thisideamosifiedimplementation"
if len(plaintext) < 8:
    plaintext += "\x00" * (8 - len(plaintext))
plaintext_bytes = plaintext.encode()

# Encrypt the binary plaintext
plaintext_int = int.from_bytes(plaintext_bytes, byteorder='big')
encrypted_value = idea.encrypt(plaintext_int)

# Convert the ciphertext to a hexadecimal string for display
ciphertext_hex = hex(encrypted_value)[2:]  # Remove the '0x' prefix
