# Define SHA-256 hash function without using hashlib
def custom_sha256(data):
    ...
    return format(digest, 'x').zfill(64)

# Example usage
data = b"thisistheimplemenatationofrepeatedhash"
print("Plaintext: ", data)
num_iterations = int(input("Enter the number of iteration you want to encrypt a plaintext: "))

# Initialize the hash value with the original data
current_hash = custom_sha256(data)

# Repeatedly apply the hash function
for _ in range(num_iterations):
    current_hash = custom_sha256(bytes.fromhex(current_hash))

# Print the final hash
print(f"Final Hash (after {num_iterations} iterations): {current_hash}")
